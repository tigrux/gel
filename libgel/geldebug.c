                                            ...
                                            const gchar *func, guint n)
{
    gel_context_set_error(context, g_error_new(
        ...
        "%s: Needs at least %u argument%s", func, n, plural(n)));
}
                                   ...
                                   const gchar *func, guint n)
{
    gel_context_set_error(context, g_error_new(
        GEL_CONTEXT_ERROR, GEL_CONTEXT_ERROR_ARGUMENTS,
        "%s: Needs %u argument%s", func, n, plural(n)));
}
                                  ...
                                  const gchar *func, const gchar *prop_name)
{
    gel_context_set_error(context, g_error_new(
        ...
        "%s: No such property '%s'", func, prop_name));
}
                                ...
                                const gchar *func, const gchar *symbol)
{
    gel_context_set_error(context, g_error_new(
        ...
        "%s: Unknown symbol '%s'", func, symbol));
}
                                         ...
                                         const gchar *func, GType type)
{
    gel_context_set_error(context, g_error_new(
        GEL_CONTEXT_ERROR, GEL_CONTEXT_ERROR_TYPE,
        "%s: Type %s is not instantiatable", func, g_type_name(type)));
}
                                   ...
                                   const gchar *func, const gchar *name)
{
    ...
    gel_context_set_error(context, g_error_new(
        GEL_CONTEXT_ERROR, GEL_CONTEXT_ERROR_TYPE,
        "%s: '%s' is not a registered type", func, name));
}
                                       ...
                                       const gchar *func, const gchar *name)
{
    gel_context_set_error(context, g_error_new(
        GEL_CONTEXT_ERROR, GEL_CONTEXT_ERROR_ARGUMENTS,
        "%s: '%s' is an invalid argument name", func, name));
}
                               ...
                               const gchar *func, const gchar *name)
{
    gel_context_set_error(context, g_error_new(
        GEL_CONTEXT_ERROR, GEL_CONTEXT_ERROR_SYMBOL,
        "%s: Symbol '%s' already exists", func, name));
}
